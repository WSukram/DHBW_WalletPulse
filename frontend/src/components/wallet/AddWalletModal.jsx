...
const AddWalletModal = ({
  open, name, setName, chainType, setChainType, chainAddress, setChainAddress,
  saving, onClose, onCreate,
}) => {
  if (!open) return null;
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-surface-container rounded-xl p-6 w-full max-w-sm border border-outline-variant/30 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button onClick={onCreate} disabled={saving || !name.trim()} className="px-4 py-2 rounded-lg bg-primary text-on-primary font-label-sm text-label-sm hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
